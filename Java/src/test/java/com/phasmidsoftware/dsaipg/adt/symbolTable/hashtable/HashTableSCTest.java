...
//        assertEquals(32768, ht1.size()); // for m = 16k
    ...
    public void testPutUpdateValue() {
        ...
        // Act
        assertEquals(Integer.valueOf(1), hashTable.put("one", 11));
    ...
    /**
     * Test the `put` method to verify that adding a new key-value pair returns `null` (indicating a new key was inserted).
     */
    @Test
    public void testPutNewKey() {
        HashTable_SC<String, Integer> hashTable = new HashTable_SC<>();
        Integer result = hashTable.put("key1", 10);
        assertNull("Adding a new key must return null", result);
        assertEquals(Integer.valueOf(10), hashTable.get("key1"));
    }

    /**
     * Test the `put` method to verify that adding a value to an existing key returns the old value.
     */
    @Test
    public void testPutExistingKey() {
        HashTable_SC<String, Integer> hashTable = new HashTable_SC<>();
        hashTable.put("key1", 10);
        Integer oldValue = hashTable.put("key1", 20);
        assertEquals("Adding a value to an existing key must return the old value", Integer.valueOf(10), oldValue);
        assertEquals(Integer.valueOf(20), hashTable.get("key1"));
    }

    /**
     * Test the `put` method to verify that keys with the same hash code handle collisions and store data correctly.
     */
    @Test
    public void testPutCollidingKeys() {
        HashTable_SC<Integer, String> hashTable = new HashTable_SC<>(3);
        hashTable.put(1, "value1");
        hashTable.put(4, "value4"); // Collides with key 1 if m=3
        assertEquals("value1", hashTable.get(1));
        assertEquals("value4", hashTable.get(4));
    }

    /**
     * Test the `put` method to verify that updating a key with collisions updates the correct key.
     */
    @Test
    public void testPutUpdateCollidingKey() {
        HashTable_SC<Integer, String> hashTable = new HashTable_SC<>(3);
        hashTable.put(1, "value1");
        hashTable.put(4, "value4"); // Collides with key 1 if m=3
        hashTable.put(1, "newValue1"); // Update key 1
        assertEquals("newValue1", hashTable.get(1));
        assertEquals("value4", hashTable.get(4));
    }
