...
public class WQUPC {
    ...
    /**
     * The parent array represents the structure of the disjoint-set (union-find).
     * Each index `i` corresponds to an element in the union-find structure, and the value
     * at `parent[i]` indicates the parent of the element `i` in the disjoint-set tree.
     * <p>
     * - If `parent[i] == i`, it means that `i` is the root of its component.
     * - The parent array is used to track and merge components during union-find operations.
     * - This array is dynamically updated as elements are merged into common components.
     */
    private final int[] parent;
    /**
     * The size array maintains the size of each subtree in the union-find data structure.
     * Each index corresponds to a node, and each value represents the size of the subtree
     * rooted at that node. It helps optimize the union operation by ensuring smaller trees
     * are connected under larger trees, thereby reducing the height of the resulting tree.
     */
    private final int[] size;
    /**
     * Represents the number of components in the union-find data structure.
     * This value indicates how many disjoint components currently exist.
     * It decreases over time as components are merged.
     */
    private int count;
}