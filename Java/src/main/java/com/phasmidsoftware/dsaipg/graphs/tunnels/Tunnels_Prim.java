...
public class Tunnels_Prim implements Iterable<Edge<Building, TunnelProperties>> {

    public static void main(String[] args) throws IOException {
        Tunnels_Prim ts = new Tunnels_Prim(BuildingLoader.createBuildings());
        double totalCost = 0.;
        double totalLength = 0.;
        GeoPrim<Building, TunnelProperties> Prim = (GeoPrim<Building, TunnelProperties>) ts.getPrim();
        Geo<Building, TunnelProperties> mst = Prim.getGeoMST(new GeoGraphSpherical<>());
    ...
    public Tunnels_Prim(List<Building> buildings) {
        setupZones();
        setupTunnels();
        // We arbitrarily limit the the length of any tunnel to 250m
        Prim = new GeoPrim<>(createGraph(buildings, e -> e.getAttribute().length <= 250));
    }

    public Graph<Building, Edge<Building, TunnelProperties>> getMst() {
        return getPrim().getMST();
    }
    ...
    private com.phasmidsoftware.dsaipg.graphs.gis.Prim<Building, TunnelProperties> getPrim() {
        return Prim;
    }

    private final Prim<Building, TunnelProperties> Prim;
